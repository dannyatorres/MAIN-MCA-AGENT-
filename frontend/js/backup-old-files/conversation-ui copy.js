...
class ConversationUI {
            ...
            container.innerHTML = `
                <div class="empty-state">
                    <div class="empty-message">
                        <p>No messages yet</p>
                        <span>Start a conversation to see messages here</span>
                    </div>
                </div>
            `;
            return;
    ...
    renderMessage(message) {
        ...
        if (message.created_at || message.timestamp) {
            ...
            if (!isNaN(messageDate.getTime())) {
                timestamp = this.formatTime(messageDate);
            }
        }
    ...
    async qualifyLenders() {
        ...
        try {
            ...
            this.showNotification('Failed to start lender qualification', 'error');
        }
    }
    ...
    async addNewLead() {
        ...
        try {
            ...
            this.showNotification('Failed to add lead: ' + error.message, 'error');
        }
    }
    ...
    switchIntelligenceTab(tab) {
        console.log(`🔍 [DEBUG] switchIntelligenceTab called with: ${tab}`);
        console.log(`🔍 [DEBUG] selectedConversation:`, this.selectedConversation);
        console.log(`🔍 [DEBUG] currentConversationId:`, this.currentConversationId);
        ...
        if (!content || !this.selectedConversation) {
            console.log(`❌ [DEBUG] Missing content (${!!content}) or selectedConversation (${!!this.selectedConversation})`);
            return;
        }
    ...
    renderDocumentsTab(content) {
        console.log('🔍 [DEBUG] renderDocumentsTab called');
        console.log('🔍 [DEBUG] selectedConversation:', this.selectedConversation);
        console.log('🔍 [DEBUG] currentConversationId:', this.currentConversationId);
                ...
                <div class="fcs-generation-section" id="fcsGenerationSection" style="display: block; margin-bottom: 20px; padding: 15px; background: #f0f9ff; border-radius: 8px; border: 1px solid #0ea5e9;">
                    <div style="display: flex; justify-content: space-between; align-items: center;">
                        ...
                        <button id="generateFCSBtn" 
                                ...
                                data-conversation-id="${this.currentConversationId || this.selectedConversation?.id || ''}"
        ...
        // Load documents and set up functionality
        this.loadDocuments();
                    ...
                    <div class="form-section">
                        ...
                        <div class="form-row-six">
                            ...
                            <div class="form-group">
                                ...
                                <input type="date" name="businessStartDate" value="${this.formatDateForInput(leadDetails.business_start_date || conv.business_start_date)}" class="form-input">
                            </div>
                            <div class="form-group">
                    ...
                    <div class="form-section">
                        ...
                        <div class="form-row-six">
                            ...
                            <div class="form-group">
                                ...
                                <input type="date" name="fundingDate" value="${this.formatDateForInput(leadDetails.funding_date)}" class="form-input">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-section">
                        ...
                        <div class="form-row-six">
                            ...
                            <div class="form-group">
                                ...
                                <input type="date" name="ownerDOB" value="${this.formatDateForInput(leadDetails.date_of_birth || conv.date_of_birth || conv.owner_dob || conv.owner_date_of_birth)}" class="form-input">
                            </div>
                        </div>
                    </div>
    ...
    updateStats(stats) {
        ...
        if (lastUpdated) {
            lastUpdated.textContent = `Last updated: ${this.formatTime(new Date())}`;
        }
    ...
    formatDateForInput(dateValue) {
        // Guard against all falsy and problematic values
        if (!dateValue || 
            dateValue === 'null' || 
            dateValue === 'undefined' || 
        ...
        try {
            ...
            // THIS IS THE KEY CHECK - verify the date is valid BEFORE calling any Date methods
            ...
            // Now we know the date is valid, we can safely call toISOString()
            const year = date.getFullYear();
            ...
            console.error('formatDateForInput caught error:', error, 'Input was:', dateValue);
            return '';
        }
    }
    ...
    getConversationIdFromDocument(documentId) {
        console.log('🔍 Trying to get conversation ID from document:', documentId);

        if (this.currentDocuments) {
            const doc = this.currentDocuments.find(d => d.id === documentId);
            if (doc && doc.conversation_id) {
                console.log('✅ Found conversation ID in document:', doc.conversation_id);
                return doc.conversation_id;
            }
        }

        console.log('❌ Could not find conversation ID from document');
        return null;
    }

    syncConversationContext() {
        console.log('🔄 Syncing conversation context...');
        ...
        if (validId) {
            ...
            console.log('✅ Conversation context synced:', validId);
            return validId;
        }
    ...
    async loadDocuments() {
        ...
        if (!this.selectedConversation) {
            console.log('❌ [DEBUG] No selectedConversation, returning early');
            return;
        }
    ...
    renderDocumentsList(documents = null) {
        ...
        console.log('📌 Conversation ID for render:', conversationId);
    ...
    setupDocumentActionListeners() {
        console.log('🎯 Setting up document action listeners');
        ...
        newDocumentsList.addEventListener('click', (event) => {
            ...
            console.log('🖱️ Document action clicked:', {
            ...
            if (convId && !this.currentConversationId) {
                ...
                console.log('📌 Set currentConversationId from button:', convId);
            }
        ...
        console.log('✅ Document action listeners set up');
    }
    ...
    async previewDocument(documentId) {
        console.log('🔍 previewDocument called');
        ...
        console.log('📌 Final conversationId resolved to:', conversationId);
    ...
    async saveDocumentEdit(documentId) {
        console.log('📝 saveDocumentEdit called for document:', documentId);
    ...
    enableInlineEdit(documentId) {
        console.log('📝 Enabling inline edit for document:', documentId);
        ...
        console.log('📝 Inline edit enabled, original name:', originalName);
    ...
    async debugDocumentUpdate(documentId) {
        console.log('🔍 DEBUG: Testing document update endpoint');
    ...
    async generatePDFApplication() {
        ...
        try {
            ...
            const applicationData = {
                ...
                dateBusinessStarted: this.formatDateForDisplay(conv.business_start_date),
                ...
                ownerDOB: this.formatDateForDisplay(conv.date_of_birth),
...
Best regards`;
    ...
    async saveLender() {
        ...
        try {
            ...
            this.showNotification('Failed to add lender: ' + error.message, 'error');
        }
    }
...
window.openAddLeadModal = function() {
    try {
        console.log('🖱️ Add Lead button clicked - opening modal');
        const modal = document.getElementById('addLeadModal');
