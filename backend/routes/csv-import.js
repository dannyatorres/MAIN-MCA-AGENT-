    ...
    try {
        ...
        for (const lead of validLeads) {
            ...
            if (existing) {
                ...
                if (isOwnLead) {
                    // Own lead — enrich with new CSV data
                    toUpdate.push({ lead, existingId: existing.id, claim: false });
                } else {
                    // Different broker — check exclusivity on submitted deals
                    const lockedStates = ['SUBMITTED', 'FUNDED', 'OFFER_RECEIVED'];
                    const isLocked = lockedStates.includes(existing.state);
                    ...
                    if (isLocked && !isExpired) {
                        ...
                        // Not locked or exclusivity expired — give them their own fresh copy
                        toInsert.push(lead);
                    }
                }
            } else {
